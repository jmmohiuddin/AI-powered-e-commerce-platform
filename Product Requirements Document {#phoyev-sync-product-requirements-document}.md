# Phoyev Sync — Product Requirements Document

**One stock pool across noon, Amazon.ae, phoyev.ae and the shop counter.**
...
| **Related** | Phoyev Commerce — Product & Technical Master Document (reverse\-engineered audit, 9 Aug 2026) |
...
**The honest constraint.** Six of twelve admin sections in Phoyev are unbuilt, including product create/edit. There are no database backups, no error tracking, no payment webhooks, and no outbound email. **A sync engine on top of that foundation would amplify failures rather than remove them.** §14 defines a hard prerequisite gate: Phase 0 ships before any channel adapter is written.
...
The 9 Aug 2026 audit found genuinely strong correctness foundations, and Phoyev Sync depends on all of them:

| Existing capability | Why Phoyev Sync needs it |
| --- | --- |
...
**This is the reason to extend Phoyev rather than start again.** The hard parts — correctness under concurrency, gapless numbering, ledger\-derived state — are done and tested.
...
| An `@phoyev/ai` package whose Anthropic client is dead code | Positioning chosen before the problem was defined |
...
| **Phoyev storefront** | Direct write inside the order transaction | n/a | Immediate | Same transaction |
...
- **Today's workflow:** a sale happens somewhere → he remembers, or does not → later, he opens noon Seller Lab, Amazon Seller Central and the Phoyev admin in three tabs and edits the same number three times.
...
- **Design consequence:** the counter\-sale flow must be **faster than not using it.** If recording a shop sale in Phoyev takes longer than writing it in a notebook, the notebook wins and the entire stock pool is corrupted at the source. This is the highest\-risk adoption dependency in v1.

### 7\.3 Tertiary — the Marketplace Shopper *(inferred)*

Buys on noon or Amazon. Never sees Phoyev. Their only relationship to this product is negative: they experience it when an order is cancelled after purchase.
...
1. **One number is true, and it lives in Postgres.** Every channel is a projection of the Phoyev stock ledger. No channel is ever consulted to answer "how many do we have."
...
**Loop 2 — Inbound orders.** Notification arrives → dedupe against the event ledger → fetch authoritative state from the channel API → create the order in Phoyev and consume the reservation → the resulting stock change re\-enters Loop 1 for every *other* channel.
...
| ID | Requirement | Acceptance criteria |
| --- | --- | --- |
| **S\-01** | **Single system of record** | Given a stock change from any source, then the Phoyev stock ledger is updated inside the originating transaction, and no channel is ever read to determine on\-hand quantity |
...
| ID | Requirement | Acceptance criteria |
| --- | --- | --- |
| **O\-01** | Amazon order ingestion via `ORDER_CHANGE` | Given an `ORDER_CHANGE` notification, then within 60 s the order exists in Phoyev with items, quantities, channel reference and channel fees, and stock is consumed. **Polling `searchOrders` at 0.0056 req/s is not an acceptable primary path** |
...
| **O\-05** | Fulfilment push\-back | When an order is marked shipped in Phoyev, the tracking reference is written back to the originating channel within 5 minutes |
...
| **SoR** | System of record — here, the Phoyev Postgres stock ledger |
